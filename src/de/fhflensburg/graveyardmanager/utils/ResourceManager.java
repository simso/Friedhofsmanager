...
public class ResourceManager {
	...
	private static final String NAMESPACE = "/de/fhflensburg/graveyardmanager";
	private static final String TILESET_LOCATION = "res" + NAMESPACE + "/maps";
	...
	public static void init(String jarLocation) throws IOException, SlickException {
		...
		while (e.hasMoreElements()) {
			...
			if (path.startsWith("res" + NAMESPACE) && path.length() > 36) {
				String dataInfo = path.split("/")[4];
				if (!path.endsWith("/")) {
					...
					else if (dataInfo.equals("sprites"))
					{
					...
					else if (dataInfo.equals("sounds"))
					{
						list.add(new DeferredSound(name, path));
					}
					else if (dataInfo.equals("musics"))
					{
						list.add(new DeferredMusic(name, path));
					}
					else if (dataInfo.equals("maps") && path.endsWith(".tmx"))
					{
						list.add(new DeferredMap(name, path, TILESET_LOCATION));
					}
					else if (dataInfo.equals("fonts") && !path.endsWith(".png"))
					{
